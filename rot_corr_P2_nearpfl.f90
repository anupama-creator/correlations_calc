...
!=================================================================================
       ...
       real(kind=8) :: maxdist,maxdist_sq,val,lent
       integer :: im,jm,ja,ii,m
       real(kind=8) :: dr_oo(3),r_oo,dr_odhd(3),r_odhd
       integer :: rgw,n ! 
       ...
       real(kind=8),allocatable :: OWHW1_vect(:,:,:) ! OH1 vect of water to check for orientation
       real(kind=8),allocatable :: length(:,:)  ! length of OH1 vector
       real(kind=8) :: dr(3)
       integer :: j1,mk
       integer :: i,j,k
       integer :: nstep
       ...
       real(kind=8),allocatable :: rot_tau(:),rot(:),rot2_tau(:),rot2(:)
...
!-------------------------------------------------------------------------------
            ...
            allocate(OWHW1_vect(fr_start:fr_end,nmol_water,3))!,length(fr_start:fr_end,nmol_water))
            ht_value = 0
            atom_cbw = 0
            nread = 0
...
!-------------------------------------------------------------------------------
      ...
      if (adjustl(atnm_rPFL(jm,7))=="O1") then ! to check 7th atom is oxygen
             ...
             dr_odhd(:) = rw(ii,2,:)-rw(ii,1,:)      ! bond length of covalently bonded OH vect
             dr_odhd(:) = dr_odhd(:)-boxl(:)*anint(dr_odhd(:)/boxl(:))
             r_odhd = (sqrt(dot_product(dr_odhd,dr_odhd)))
             OWHW1_vect(t,imol_water,:) = (dr_odhd(:)/r_odhd) ! OH vect
             !length(t,imol_water) = r_odhd           ! vect length
     endif
     enddo
     deallocate(xx1,yy1,zz1)
     enddo nframe1
     ...
     allocate(rot_tau(0:fr_end-fr_start), norm(0:fr_end-fr_start),rot(0:fr_end-fr_start))           ! allocate the ones required for calculation
     allocate(rot2_tau(0:fr_end-fr_start),rot2(0:fr_end-fr_start))
         rot_tau(:) = 0.0
         rot2_tau(:) = 0.0
         norm(:) = 0.0
         rot(:) = 0.0
...
!--------------------------------------------------------------------------------
       ...
       val = dot_product(OWHW1_vect(k,imol_water,:),OWHW1_vect(i,imol_water,:))
       rot(j)  = rot(j)  + (val)
       rot2(j) = rot2(j) + 0.5*(3*(val*val)-1)
       norm(j) = norm(j) + 1.0d0
    !   write(24,*) "norm(",j,") = ",norm(j)
    !   write(23,*) "rot_tau(",j,")  =  ",rot_tau(j)," + ",rsq
       endif
       endif
       enddo lp
       enddo 
       enddo br2
       deallocate(OWHW1_vect,ht_value)!,length)
!===============================================================================
                 rot_tau(0)  = rot(0)/norm(0)
                 rot2_tau(0) = rot2(0)/norm(0)
!===============================================================================
                ...
                if (norm(i) == 0.0d0) then
                   ...
                   rot_tau(i)  = (rot(i)/norm(i))/rot_tau(0)
                   rot2_tau(i) = (rot2(i)/norm(i))/rot2_tau(0)
                endif
                !m = int((i*5)/1000)
                write(4,1011) real(i)*50*0.002, rot_tau(i)! *0.001*100
                write(5,1011) real(i)*50*0.002, rot2_tau(i)
1011  format(F12.7,F12.7)
                enddo
               call trj%close()
               deallocate(rot,rot2,rot_tau,rot2_tau,norm)
               enddo input
!-------------------------------------------------------------------------------
               call cpu_time(finish)
               print'("Time= ",f6.3,"seconds")',finish-start,finish,start
               end program orientational
!program was closed
